...
<div style="padding: 0px 16px">
    ...
    <div style="overflow: hidden; padding: 8px; background: #f5f7fa; margin-top: 16px">
        <div style="float: left; width: 85px; word-break: break-all">收费描述：</div>
        <div style="margin-left: 85px"> ${remark!} </div>
    </div>
...
<div style="position: relative; padding: 0px 16px; padding-bottom: 16px">
    ...
    <div style="overflow: hidden; margin-bottom: 8px">
        <div style="float: left">缴费日期：<span style="font-weight: 400; color: #606266">${payTime!'    '}</span></div>
        <#if signStatus?? && signStatus == 0>
            <div style="float: right;margin-right:80px">开具日期：<span style="font-weight: 400; color: #606266">${billingTime!}</span></div>
        <#else>
            <div style="float: right;">开具日期：<span style="font-weight: 400; color: #606266">${billingTime!}</span></div>
        </#if>

    </div>
    <div style="overflow: hidden; margin-bottom: 8px">
        <div style="float: left">开票人：<span style="font-weight: 400; color: #606266">${clerk!}</span></div>
        <#if signStatus?? && signStatus == 0>
            <div style="float: right">收款单位（盖章）：<span style="font-weight: 400; color: white;">熊仔王盖章点</span></div>
        <#else>
            <div style="float: right">收款单位（盖章）：<span style="font-weight: 400; color: #606266">${statutoryBodyName!}</span></div>
        </#if>

    </div>
    <#if enableElectSign?? && (enableElectSign=1) && electSignType?? && (electSignType=2)>
        <div>
            <img style="width: 90px; height: 90px; opacity: 0.7;float: right" src="${signPictureUrl!}" />
        </div>
    </#if>
</div>
</body>
