...
public class DateTimeUtil {
    ...
    public static final DateTimeFormatter YEAR_MONTH_DAY_FORMAT = DateTimeFormatter.ofPattern(YEAR_MONTH_DAY_PATTERN);
    ...
    public static String getYearMonthDay(LocalDateTime dateTime) {
        return dateTime.format(YEAR_MONTH_DAY_FORMAT);
    }
     ...
     * @return
     */
    public static String handleStartEndTimeDate(List<StartEndTimeDTO> startEndTimeDates) {
        Map<String, ArrayDeque> dequeData = groupByContinuouslyBillTimes(startEndTimeDates, "startTime", "endTime", StartEndTimeDTO.class);
        String startEndTime = "";
        if (dequeData != null && !dequeData.isEmpty()) {
            for (String key : dequeData.keySet()) {
                StartEndTimeDTO first = (StartEndTimeDTO) dequeData.get(key).getFirst();
                StartEndTimeDTO last = (StartEndTimeDTO) dequeData.get(key).getLast();
                LocalDateTime startTime = first.getStartTime();
                LocalDateTime endTime = last.getEndTime();
                startEndTime = startEndTime + startTime.toLocalDate() + "-" + endTime.toLocalDate() + ";";
            }
        }
        return startEndTime;
    }
