...
/**
 * description 资金收款handler
 * @author Yuting.Wang
 * @since 2024/12/18 15:44
 */
@Component
@Slf4j
public class FundReceiptHandler extends AbustractPushBillHandler{
    ...
    @Autowired
    private  FlowClaimRecordRepository flowClaimRecordRepository;
    @Autowired
    ...
    @Resource
    private Mdm63LockMapper mdm63LockMapper;
    @Resource
    private VoucherBillZJRecDetailMapper voucherBillZJRecDetailMapper;
    ...
    public boolean support(ApproveProcessCompleteMsg param) {
        return OperateTypeEnum.资金收款.getDes().equals(param.getType());
    }
    ...
    public void approveRefuse(Long voucherBillId, String procInstId) {
        log.info("资金收款审批驳回：FundReceiptHandler.approveRefuse");
        ...
        //根据报账单ID删除锁定MDM63数据
        mdm63LockMapper.updateDeletedByVoucherBIllId(voucherBillId);
        //根据报账单编号删除应收款明细数据
        voucherBillZJRecDetailMapper.updateDeletedByVoucherBIllNo(voucherBillZJ.getVoucherBillNo());
    }
