...
@Service
@Slf4j
public class AdvanceReceiveChargeAgainstStrategy extends ManualVoucherStrategy<ManualVoucherStrategyCommand>{


    public AdvanceReceiveChargeAgainstStrategy() {
        super(VoucherEventTypeEnum.预收应收核销冲销);
    }
    ...
    public List<Voucher> makeVoucher(VoucherRule rule, VoucherTemplate template, VoucherMakeError voucherMakeError,
        ...
        for (VoucherBusinessBill businessBill : businessBills) {
            ...
            if (StrUtil.isBlank(businessBill.getSceneId())){
                log.error(VoucherEventTypeEnum.预收应收核销冲销.getValue()+"异常，sceneId为空,入参:{}", JSONObject.toJSONString(businessBill));
                continue;
            }
            ...
            List<VoucherBusinessDetail> voucherBusinessDetailsList = voucherBusinessDetailRepository.getBaseMapper()
                    .selectList(Wrappers.<VoucherBusinessDetail>lambdaQuery()
                            ...
                            .eq(VoucherBusinessDetail::getSceneType,VoucherEventTypeEnum.预收应收核销.getCode())
            ...
            for (int i = 0; i < voucherBusinessDetailsList.size(); i++) {
                ...
                voucherBusinessBill.setSceneType(VoucherEventTypeEnum.预收应收核销冲销.getCode());
    ...
    public List<VoucherBusinessBill> businessBills(ManualVoucherStrategyCommand command, List<VoucherRuleConditionOBV> conditions) {
        return voucherFacade.getChargeAgainstBillList(command,conditions,VoucherEventTypeEnum.预收应收核销.getCode(),VoucherEventTypeEnum.预收应收核销冲销.getCode());
    }
}
