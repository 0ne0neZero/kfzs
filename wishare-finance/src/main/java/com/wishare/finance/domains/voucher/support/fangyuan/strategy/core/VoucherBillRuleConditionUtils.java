 ...
 * @since 2023/4/10
 */
public class VoucherBillRuleConditionUtils {
    ...
    public static QueryWrapper<?> parseConditionToQuery(List<VoucherBillRuleConditionOBV> conditions, BillTypeEnum billTypeEnum) {
        ...
        for (VoucherBillRuleConditionOBV condition : conditions) {
            FyVoucherRuleConditionTypeEnum ruleConditionTypeEnum = FyVoucherRuleConditionTypeEnum.valueOfByCode(condition.getType());
            switch (ruleConditionTypeEnum) {
                ...
                case 单据来源:
                    ...
                    break;
                case 费用类型:
                    putField(condition.getMethod(), queryWrapper, "b.bill_cost_type",
                            condition.getValues().stream().map(VoucherRuleConditionOptionOBV::getId).collect(Collectors.toList()), false);
                    break;
                case 结算时间:
                    putField(condition.getMethod(), queryWrapper, "b.bill_type",
                ...
                case 收款时间:
                    putField(condition.getMethod(), queryWrapper, "gd.pay_time",
                            condition.getValues().stream().map(VoucherRuleConditionOptionOBV::getId).collect(Collectors.toList()), true);
                    break;
                case 归属月:
                    ...
                    putField(condition.getMethod(), queryWrapper, "b.account_date",
                            dates, true);
                    break;
                case 结算方式:
    ...
    private static QueryWrapper<?> putField(Integer method, QueryWrapper<?> wrapper, String field, List<Object> values, boolean isDate) {
        switch (VoucherRuleConditionMethodEnum.valueOfByCode(method)) {
            ...
            case 大于等于:
                ...
                break;
            case 小于:
                wrapper.lt(field, values.get(0));
                break;
            case 大于:
                wrapper.gt(field, values.get(0));
                break;
        }
        return wrapper;
    }
