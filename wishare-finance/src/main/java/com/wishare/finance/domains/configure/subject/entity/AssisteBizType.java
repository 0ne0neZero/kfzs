...
@TableName(value = TableNames.ASSISTE_BIZ_TYPE)
@ApiModel(value="AssisteBiztype对象", description="辅助核算（业务类型）")
public class AssisteBizType implements Serializable {
    ...
    public void updateOrInsert(){
        if(id == null){
            id = IdentifierFactory.getInstance().generateLongIdentifier(TableNames.ASSISTE_BIZ_TYPE);
        }
        if (disabled == null){
            disabled = 0;
        }
