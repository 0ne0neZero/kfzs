 ...
 */
@Component
public class ActualPayNoFeeLogicStrategy implements LogicStrategy {

    @Override
    public VoucherTemplateLogicCodeEnum logicCode() {
        return VoucherTemplateLogicCodeEnum.实收付金额不含手续费;
    }

    @Override
    public BigDecimal logicValue(VoucherBusinessBill businessBill) {
        BigDecimal actualPayAmount = AmountUtils.longToDecimal(businessBill.getActualPayAmount()) ;
        return actualPayAmount.subtract(AmountUtils.longToDecimal(businessBill.getFee()));
    }
