...
@Getter
@ApiModel("未开票应收单信息")
public class UnInvoiceReceivableBillDto extends Bill {
    ...
    public String getPayChannelName(){
        if (StringUtils.isNotBlank(this.payInfos)){
            List<PayInfo> payInfos1 = JSON.parseArray(payInfos, PayInfo.class);
            return payInfos1.stream().map(info -> SettleChannelEnum.valueNameOfByCode(info.getPayChannel()))
    ...
    public Long getSettleAmount() {
        return getReceivableAmount() - getDiscountAmount();
    }
