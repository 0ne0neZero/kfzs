...
@Slf4j
public class PayExportService implements IStandardImport<PayContractImport>, IOwlApiBase {
    ...
    private final ContractPayConcludeMapper contractPayConcludeMapper;
    ...
    public ImportStandardV customerImport(MultipartFile excelFile) {
        importTaskService.executeTask(
                ImportTaskEnum.PAY_CONTRACT_IMPORT, excelFile, inputStream -> {
                    DataImportBuilder<PayContractImport> importBuilder = DataImportFactory.create();
                    Map<String, Object> customMap = new HashMap<>();
                    return importBuilder.createFrom(inputStream, PayContractImport.class, 1)
                            .with(customMap)
                            .standard(this);
                });
    ...
    @Override
    public void execute(PayContractImport next, DataImportContext<PayContractImport> dataImportContext) {

    }

    @Override
    public List<ErrorRow<PayContractImport>> save(List<PayContractImport> dataList, DataImportContext<PayContractImport> dataImportContext) {

        for (PayContractImport map : dataList) {

            ContractPayConcludeE add = new ContractPayConcludeE();
            ...
            String code = contractConcludeAppService.revContractCode(tenantId(), null, RevTypeEnum.支出合同.getCode());
            ...
            try {
                contractPayConcludeMapper.insert(add);
    ...
    @Override
    public List<ErrorColumn> isSatisfied(PayContractImport next, DataImportContext<PayContractImport> dataImportContext) {
    ...
    private static final String modelName = "支出合同列表数据导入模板.xlsx";
    ...
    public FileVo download() {
        try {
            ...
            PayContractImport importD = new PayContractImport();

            Map<Integer, String[]> nature = new HashMap<>();
            nature.put(3, new String[]{"普通合同", "框架合同", "补充协议"});
            EasyExcel.write(file, PayContractImport.class)
