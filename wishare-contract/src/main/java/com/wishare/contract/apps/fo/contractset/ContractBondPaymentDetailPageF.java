...
public class ContractBondPaymentDetailPageF {
    ...
    @ApiModelProperty("需要查询返回的字段，不传时返回以下全部，可选字段列表如下"
        ...
        + "contractId 合同Id"
        + "bondPlanId 保证金计划id"
        + "paymentAmount 申请付/退款金额（元）"
        + "paymentMethod 付/退款方式  0现金  1银行转帐  2汇款  3支票"
        + "auditCode 关联审批编号"
        + "auditStatus 审批状态 0通过  1审核中  2未通过"
        + "confirmStatus 财务状态  0已确认  1未确认"
        + "applyPaymentTime 申请付/退款时间"
        + "actualPaymentTime 实际付/退款时间"
        + "remark 备注"
        + "deleted 是否删除"
        + "creator 创建人ID"
        + "creatorName 创建人姓名"
