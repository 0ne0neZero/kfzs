...
public class ContractPayConcludeTreeV extends Tree<ContractPayConcludeTreeV,String> {
    ...
    */
     ...
     * 是否展示打印按钮
     */
    @ApiModelProperty("是否展示合同签订")
    private Boolean showBtnSign;

    @ApiModelProperty("是否展示重复推送")
    private Boolean showBtnRepeatPull;
    /**
     * 是否展示删除按钮
     */
    ...
    @ApiModelProperty("合同管理类别描述")
    private String conmanagetypeName;

    @ApiModelProperty("是否开始NK")
    private Boolean isStartNK = Boolean.FALSE;

    @ApiModelProperty("是否结束NK")
    private Boolean isEndNK = Boolean.FALSE;

    @ApiModelProperty("是否展示修正")
    private Boolean isShowXz = Boolean.FALSE;

    //Nk状态（0.未开启；1.已开启;2.已关闭；3.关闭中）
    private Integer nkStatus;
    //结束NK时BPM审批流程，若有值，则展示查看流程按钮
    private String showNkBpm;
    //BPM流程ID
